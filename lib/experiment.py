...
class Experiment:
    ...
    def load_last_train_state(self, model, optimizer, scheduler, device=None):
        ...
        # for k, v in train_state['model'].items():
        #     print(k)
        ###### pop cls_layer  for  multly lane type
        model_dict = model.state_dict()
        pop_cls_layer = {k: v for k, v in train_state['model'].items() if 'cls_layer' not in k}
        model_dict.update(pop_cls_layer)
