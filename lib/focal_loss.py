...
if __name__ == '__main__':
    ...
    focal_losshh = FocalLoss(alpha=0.25, gamma=2.)
    aa = np.array([0,1,1], dtype=np.int64)
    cls_target = torch.from_numpy(aa)
    bb = np.array([[0.01, 0.46, 0.09],[0.01, 0.46, 0.09],[0.01, 0.46, 0.09]],dtype=np.float)
    cls_pred = torch.from_numpy(bb)
    cls_loss = focal_losshh(cls_pred, cls_target).sum()
    print(cls_loss)
